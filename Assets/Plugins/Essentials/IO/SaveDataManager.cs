...
namespace Skeletom.Essentials.IO
{
	...
	/// <summary>
	/// Manager for handling Read/Write operations.
	/// </summary>
	public class SaveDataManager : Singleton<SaveDataManager>
	{
		...
		public bool WriteImage(string directory, string filename, Texture2D image)
		{
			try
			{
				...
				return WriteFile(directory, filename, bytes);
			}
			catch (Exception e)
			{
				Debug.LogError(string.Format("Error encoding image: {0} - {1}", directory, e));
			}
			return false;
		}

		public bool WriteFile(string directory, string fileName, string content)
		{
			try
			{
			...
			{
				Debug.LogError(string.Format("Error writing file to directory: {0} - {1}", directory, e));
			}
			return false;
		}

		public bool WriteFile(string directory, GenericFile file)
		{
			try
			{
				byte[] bytes = file.fileContent.bytes;
				return WriteFile(directory, file.fileName, file.fileContent.bytes);
			}
			catch (Exception e)
			{
				Debug.LogError(string.Format("Error writing file to directory: {0} - {1}", directory, e));
		...
		private bool WriteFile(string directory, string filename, byte[] bytes)
		{
			try
			{
